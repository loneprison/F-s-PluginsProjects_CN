...
#include "SparkRing.h"
#include "SparkRingText.generated.h"
	...
	PF_LayerDef		*output)
{
	...
	PF_ParamDef		def;
	const SparkRingText::Strings strings(in_data);
	...
	//位置の指定
	AEFX_CLR_STRUCT(def);
	PF_ADD_POINT(AETEXT_PARAM(strings, L10N_PARAM_OFFSET),			/*"New Center"*/
		50,	// X
		50,	// Y
		0,	// Flag
		ID_OFFSET
	...
	//----------------------------------------------------------------
	AEFX_CLR_STRUCT(def);
	PF_ADD_ANGLE(AETEXT_PARAM(strings, L10N_PARAM_POINT_ROT), 0, ID_POIN_ROT);
	//----------------------------------------------------------------
	//----------------------------------------------------------------
	AEFX_CLR_STRUCT(def);
	PF_ADD_SLIDER(AETEXT_PARAM(strings, L10N_PARAM_POINT_RAND),	//パラメータの名前
		0, 				//数値入力する場合の最小値
		300,			//数値入力する場合の最大値
		0,				//スライダーの最小値 
		100,				//スライダーの最大値
