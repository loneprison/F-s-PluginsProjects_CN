	...
	PF_LayerDef		*output)
{
	...
	//色の指定
	AEFX_CLR_STRUCT(def);
	PF_ADD_COLOR(	AETEXT_PARAM(strings, L10N_PARAM_MAIN_COLOR),
					0x00,
					0x00,
					0x00,
					MY_Main_Color
	...
	PF_LayerDef		*output )
{
	...
	if  ( exe.Enabled() == FALSE ) {
		...
		PF_SPRINTF(	out_data->return_msg, 
					"[%s] %s",
					STR(StrID_Name),
					AETEXT_ERROR(strings, L10N_ERR_GETPARAMS)
					);
		return err;
	}
	...
	void			*extra)
{
	...
	switch (cmd) {
		...
		case PF_Cmd_DO_DIALOG: {
			MainLineRepaint_oldText::OpenSettings(in_data, L"F's MainLineRepaint_old");
			break;
		}
