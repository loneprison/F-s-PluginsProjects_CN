...
#include "NoiseHiLo_Alpha.h"
#include "NoiseHiLo_AlphaText.generated.h"
	...
	PF_LayerDef		*output)
{
	...
	PF_ParamDef		def;
	const NoiseHiLo_AlphaText::Strings strings(in_data);
	...
	PF_Pixel8	*outP)
{
	...
	PF_FpLong v = FS_CONVERT8TO32(p.alpha);
	...
	if (a != 0){
		...
		if(v>st){
			...
			p.alpha	= RoundByteLong((A_long)p.alpha+aa);
		}
	}
	m *= niP->mdLv;
	if (m != 0){
		...
		if((v>st)&&(v<lt)){
			...
			p.alpha	= RoundByteLong((A_long)p.alpha+mm);
		}
	}
	...
	if (b != 0){
		...
		if(v<lt){
			...
			p.alpha	= RoundByteLong((A_long)p.alpha+bb);
		}
	}
	*outP = p;
	return err;
	...
	PF_Pixel16	*outP)
{
	...
	PF_FpLong v =  (PF_FpLong)p.alpha / PF_MAX_CHAN16;
	if((v<=0) || (v>=1.0) ) return err;
	...
	if (a != 0){
		...
		if(v>st){
			...
			p.alpha	= RoundShort((A_long)p.alpha+aa);
		}
	}
	m *= niP->mdLv;
	if (m != 0){
		...
		if((v>st)&&(v<lt)){
			...
			p.alpha	= RoundShort((A_long)p.alpha+mm);
		}
	}
	...
	if (b != 0){
		...
		if(v<lt){
			...
			p.alpha	= RoundShort((A_long)p.alpha+bb);
		}
	}
	*outP = p;
	return err;
	...
	PF_PixelFloat	*outP)
{
	...
	if (a != 0){
		...
		if(v>st){
			...
			p.alpha	= RoundFpShortDouble(p.alpha+a);
		}
	}
	m *= niP->mdLv;
	if (m != 0){
		...
		if((v>st)&&(v<lt)){
			...
			p.alpha	= RoundFpShortDouble(p.alpha+m);
		}
	}
	...
	if (b != 0){
		...
		if(v<lt){
			...
			p.alpha	= RoundFpShortDouble(p.alpha+b);
		}
	}
	*outP = p;
	return err;
