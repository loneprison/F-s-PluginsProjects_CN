...
#include "sputteringSplashText.generated.h"
	...
	PF_LayerDef		*output)
{
	...
	PF_ParamDef		def;
	const sputteringSplashText::Strings strings(in_data);
	...
	//ポップアップ
	AEFX_CLR_STRUCT(def);	
	PF_ADD_POPUP(	AETEXT_PARAM(strings, L10N_PARAM_DIRECTION),
					UI_DIR_COUNT,	//メニューの数
					UI_DIR_DFLT,	//デフォルト
					AETEXT_POPUP(strings, L10N_PARAM_DIRECTION_ITEMS),
					ID_DIR
					);
	//-----------------
	AEFX_CLR_STRUCT(def);
