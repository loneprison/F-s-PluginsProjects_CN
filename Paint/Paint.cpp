	...
	PF_LayerDef		*output )
{
	...
	CFsAE ae;
	const PaintText::Strings strings(in_data);
	...
	PF_LayerDef		*output)
{
	...
	//色の指定
	AEFX_CLR_STRUCT(def);
	PF_ADD_COLOR(	AETEXT_PARAM(strings, L10N_PARAM_PAINT_COLOR),
					0xFF,
					0x00,
					0x00,
					ID_COLOR
...
//-------------------------------------------------------------------------------------------------
static PF_Err GetParams(CFsAE *ae, PaintInfo *infoP)
{
	PF_Err		err 		= PF_Err_NONE;

	ERR(ae->GetCHECKBOX(ID_EXECUTED_CB,&infoP->executed));
	ERR(ae->GetFIXEDPOINT(ID_POS,&infoP->pos));
	ERR(ae->GetCOLOR(ID_COLOR,&infoP->color));
	ERR(ae->GetFIXED(ID_COLOR_OPACITY,&infoP->opacity));
	if (!err){
		if(!err) infoP->opacity /= 100;
		if(!err) infoP->color.alpha =  RoundByteLong( PF_MAX_CHAN8 * infoP->opacity >>16);
	}
	return err;
}
//-------------------------------------------------------------------------------------------------
static PF_Err 
	Exec (CFsAE *ae , PaintInfo *infoP)
{
	PF_Err	err = PF_Err_NONE;
	...
	if (!err){
		if (infoP->executed==TRUE){
			err = ae->out->paint(infoP->pos,infoP->color);
		}
	}
	return err;
}
...
{
	PF_Err		err 		= PF_Err_NONE;
	CFsAE ae(in_data,out_data,extraP,sizeof(PaintInfo),ID_NUM_PARAMS);
	err = ae.resultErr();
	if (!err){

		PaintInfo *infoP = reinterpret_cast<PaintInfo*>(ae.LockPreRenderData());
		if (infoP){
	...
	try
	{
		...
		switch (cmd) {
			...
			case PF_Cmd_DO_DIALOG: {
				...
				PaintText::OpenSettings(in_data, L"F's Paint");
				break;		
			}
