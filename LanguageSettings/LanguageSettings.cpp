	...
	const HostEnvironment &environment)
{
	...
	return UiText(ui_language, Text::MessageIncompatible);
}
	...
	AeText::TextResult *result)
{
	...
	result->legacy = AeText::detail::LegacyValue(*variant, request->token);
	result->utf8 = requires_utf8
...
{
	return static_cast<ConfiguredLanguage>(radio_id - 1000);
}
