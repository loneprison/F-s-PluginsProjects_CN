...
function Get-OptionalString {
    ...
    if ($item.Value -is [System.Management.Automation.PSCustomObject]) {
        Assert-Properties -Value $item.Value -Allowed @('useSource') -Name "translation $Language/$Id"
        $useSource = $item.Value.PSObject.Properties['useSource']
        if ($null -ne $useSource -and $useSource.Value -is [bool] -and $useSource.Value) {
            return $Fallback
        }
    }
    if ($item.Value -isnot [string] -or $item.Value.Length -eq 0) {
        throw "Translation must be a non-empty string or explicit useSource: language=$Language id=$Id"
    }
    return [string]$item.Value
}
...
function Get-Placeholders {
    ...
    $printf = @(
        [regex]::Matches($Value, '%%|%(?:\d+\$)?[-+ #0'']*(?:\d+|\*)?(?:\.(?:\d+|\*))?(?:hh|h|ll|l|j|z|t|L)?[diuoxXfFeEgGaAcspn]') |
            Where-Object { $_.Value -cne '%%' } |
            ForEach-Object { "printf:$($_.Value)" }
    )
    return @($named | Sort-Object) + $printf
}
