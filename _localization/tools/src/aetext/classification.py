...
class ClassificationPlan:
    ...
    review_state_moves: list[tuple[Path, Path]]
    project_updates: list[tuple[Path, bytes]]
...
def _catalog_scope(project: Path, catalog_root: Path, catalog: Path) -> tuple[str, str, Path]:
    relative = catalog.relative_to(catalog_root)
    if project.stem == "FsLanguageSettings":
        if relative != Path("Settings.json"):
            raise ClassificationError(
                f"Language Settings catalog must be directly below the catalog root: {catalog}"
            )
...
def _expected_catalog_path(
    ...
    if project.stem == "FsLanguageSettings":
        return (catalog_root / "Settings.json").resolve()
    isolation_parts = relative.parent.parts[1:]
...
def _catalog_reference_update(
    path: Path, source: Path, destination: Path
) -> bytes:
    # Change only the matching item attribute; preserve XML layout, BOM and line endings.
    text = path.read_bytes().decode("utf-8")
    pattern = re.compile(r'(<None\b[^>]*?\bInclude\s*=\s*)([\'"])(.*?)(\2)', re.DOTALL)

    def update(match: re.Match[str]) -> str:
        if _resolved_item(path, html.unescape(match[3])) != source:
            return match[0]
        relative = os.path.relpath(destination, path.parent).replace("/", "\\")
        escaped = html.escape(relative, quote=False).replace(
            match[2], "&quot;" if match[2] == '"' else "&apos;"
        )
        return match[1] + match[2] + escaped + match[4]

    return pattern.sub(update, text).encode("utf-8")
...
def build_classification_plan(root: str | Path) -> ClassificationPlan:
    ...
    for project in sorted(project_paths, key=lambda value: (value.stem.casefold(), value)):
        ...
        role, isolation, relative_catalog = _catalog_scope(project, catalog_root, catalog)
        category = "" if role == "Support" else _source_category(project, project_xml)
        expected_catalog = _expected_catalog_path(
            project, catalog_root, relative_catalog, role, category
        )
    ...
    review_root = repository / "_localization" / "review-state"
    review_moves = []
    project_updates = []
    for project in projects:
        source, destination = project.catalog_path, project.expected_catalog_path
        if source == destination:
            continue
        review_source = review_root / source.relative_to(catalog_root)
        review_destination = review_root / destination.relative_to(catalog_root)
        if review_destination.exists():
            raise ClassificationError(
                f"review-state move would overwrite an existing path: {review_destination}"
            )
        if review_source.is_file():
            review_moves.append((review_source, review_destination))
        for path in (project.project_path, project.project_path.with_suffix(".vcxproj.filters")):
            if path.is_file():
                updated = _catalog_reference_update(path, source, destination)
                if updated != path.read_bytes():
                    project_updates.append((path, updated))
    return ClassificationPlan(
        ...
        catalog_moves=moves,
        review_state_moves=review_moves,
...
def apply_classification_plan(plan: ClassificationPlan) -> None:
    moves = plan.catalog_moves + plan.review_state_moves
    for _, destination in moves:
        if destination.exists():
            raise ClassificationError(
                f"catalog move would overwrite an existing path: {destination}"
            )
    updates = dict(plan.project_updates)
    updates[plan.plugin_map_path] = plan.plugin_map_text.encode("utf-8")
    updates[plan.solution_path] = plan.solution_text.encode("utf-8")
    originals = {path: path.read_bytes() if path.exists() else None for path in updates}
    originals.update({source: source.read_bytes() for source, _ in moves})
    originals.update({destination: None for _, destination in moves})
    changed = []
    try:
        for source, destination in moves:
            _atomic_write(destination, originals[source])
