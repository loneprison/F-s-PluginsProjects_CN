...
$scratch = Join-Path ([System.IO.Path]::GetTempPath()) ("AeTextCatalogTests-" + [Guid]::NewGuid().ToString('N'))
...
function Get-CatalogPath {
    ...
    if ($ProjectName -eq 'FsLanguageSettings') {
        return Join-Path $catalogRoot 'Settings.json'
    }
    ...
    return Join-Path $directory "$ProjectName.json"
}
...
try {
    ...
    if ([System.IO.Directory]::Exists($scratch)) {
        ...
        if ($resolvedScratch -cne [System.IO.Path]::GetFullPath($scratch)) {
            throw "Unexpected validation output path: $resolvedScratch"
        }
